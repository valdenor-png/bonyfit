-- ============================================
-- BONY FIT — LOJA COMPLETA (FIX)
-- ============================================

-- Adicionar colunas que faltam
ALTER TABLE loja_produtos ADD COLUMN IF NOT EXISTS ordem INT DEFAULT 0;
ALTER TABLE loja_produtos ADD COLUMN IF NOT EXISTS preco_centavos INT;

-- Limpar dados antigos
DELETE FROM loja_produtos;
DELETE FROM loja_categorias WHERE slug NOT IN ('roupas', 'suplementos', 'acai_bone');
...
AND p.nome NOT LIKE '%Mochila%' AND p.nome NOT LIKE '%Bolsa%' AND p.nome NOT LIKE '%Boné%';

-- Mochila, Bolsa, Boné: tamanho único
INSERT INTO loja_variacoes (produto_id, nome, estoque, ordem)
...
-- Variações Suplementos (sabores para Whey e Pré-Treino)
INSERT INTO loja_variacoes (produto_id, nome, estoque, ordem)
...
-- Variações Açaí (tamanhos com preços diferentes)
...
CREATE POLICY IF NOT EXISTS "variacoes_select" ON loja_variacoes FOR SELECT USING (ativo = true);
CREATE POLICY IF NOT EXISTS "pedido_itens_select" ON loja_pedido_itens FOR SELECT USING (
  pedido_id IN (SELECT id FROM loja_pedidos WHERE user_id = auth.uid())
);
CREATE POLICY IF NOT EXISTS "favoritos_all" ON loja_favoritos FOR ALL USING (usuario_id = auth.uid());
