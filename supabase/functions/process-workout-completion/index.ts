...
const handler = createHandler(async (req, supabase) => {
  ...
  if (!record?.user_id) {
    throw { status: 400, code: 'INVALID_PAYLOAD', message: 'Payload inválido: user_id ausente' }
  }
  ...
  if (lastWorkout) {
    ...
    if (daysSince <= 2) {
      ...
      await supabase.from('users').update({ current_streak: 1 }).eq('id', userId)
    }
  }
  ...
  await supabase.rpc('add_user_points', { p_user_id: userId, p_points: finalPoints })
  ...
  try {
    ...
    if (pushToken) {
      const streakText = streakMultiplier > 1 ? ` (${streakMultiplier.toFixed(1)}x streak!)` : ''
  ...
  await logAudit(supabase, {
    ...
    entityId: record.id,
    metadata: { points: finalPoints, streak: streakMultiplier, sets: totalSets, exercises: totalExercises },
  })

  return success({ points: finalPoints, streak: streakMultiplier })
}, { functionName: 'process-workout-completion', allowedMethods: ['POST'] })
